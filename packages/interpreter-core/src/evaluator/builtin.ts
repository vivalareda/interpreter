...
const tail: BuiltinFunction = (...args: Object[]) => {
  ...
  if (args[0].Type() !== OBJECTS.ARRAY_OBJ) {
    return new Error(
      `argument to \`tail\` must be ARRAY, got ${args[0].Type()}`,
    );
  }
...
const garocheMoiCa: BuiltinFunction = (...args: Object[]) => {
...
export class Builtin implements Object {
  constructor(public func: (...args: Object[]) => Object) {}
...
export const BUILTINS: Map<string, Builtin> = new Map([
  ...
  ["GAROCHE MOI CA", new Builtin(garocheMoiCa)],
]);
